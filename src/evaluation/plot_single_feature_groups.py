...
FEATURE_SETS = [
    ...
    "only_attribute",
    "only_query_semantic",
    "only_context_semantic",
    "only_relation_semantic",
    "only_query_attributes",
    "only_context_scale",
    "only_compression_attributes",
    "only_multi_turn_markers",
    "only_language_attributes",
]

LABELS = {
    "full_features": "Full",
    "only_semantic": "Only Semantic",
    "only_attribute": "Only Attribute",
    "only_query_semantic": "Only Query Sem.",
    "only_context_semantic": "Only Context Sem.",
    "only_relation_semantic": "Only Relation Sem.",
    "only_query_attributes": "Only Query Attr.",
    "only_context_scale": "Only Context Scale",
    "only_compression_attributes": "Only Compression",
    "only_multi_turn_markers": "Only Markers",
    "only_language_attributes": "Only Language",
}
        ...
        rows.append(
            {
                ...
                "strong_call_rate": metric["strong_call_rate"],
...
def plot_roc(data, results_dir, output_dir, feature_sets):
    ...
    plt.title("ROC Curves for Single Feature Groups")
    plt.grid(alpha=0.25)
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig(output_dir / "single_feature_roc_curves.png", dpi=300)
    plt.savefig(output_dir / "single_feature_roc_curves.pdf")
    plt.close()
    return rows
...
def plot_pr(data, results_dir, output_dir, feature_sets):
    ...
    plt.title("Precision-Recall Curves for Single Feature Groups")
    plt.grid(alpha=0.25)
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig(output_dir / "single_feature_pr_curves.png", dpi=300)
    plt.savefig(output_dir / "single_feature_pr_curves.pdf")
    plt.close()
    return rows
...
def plot_cost_quality(results_dir, output_dir, feature_sets):
    ...
    plt.title("Cost-Quality Curves for Single Feature Groups")
    plt.xlim(0, 0.9)
    plt.ylim(0, 1.05)
    plt.grid(alpha=0.25)
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig(output_dir / "single_feature_cost_quality_curves.png", dpi=300)
    plt.savefig(output_dir / "single_feature_cost_quality_curves.pdf")
    plt.close()
    return rows
...
def write_summary(rows, output_dir):
    with open(output_dir / "single_feature_summary.json", "w", encoding="utf-8") as f:
    ...
    with open(output_dir / "single_feature_summary.tex", "w", encoding="utf-8") as f:
        f.write("\n".join(lines) + "\n")


def parse_args():
    parser = argparse.ArgumentParser(description="Summarize single feature group experiments.")
    parser.add_argument("--features", default="artifacts/features/recent_semantic_nall.npz")
    parser.add_argument("--results-dir", default="artifacts/results/04_single_feature_groups")
    parser.add_argument("--output-dir", default="artifacts/plots/04_single_feature_groups")
...
def main():
    ...
    print(f"Wrote single feature group summary to {output_dir}")
